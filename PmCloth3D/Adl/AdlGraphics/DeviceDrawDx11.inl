...
{
	public:
		ShaderUtilsDX11( const Device* deviceData, const char* fileName )
...
{
	DeviceDX11* deviceData = (DeviceDX11*)m_deviceData;
	...
	if( FAILED( hr ) )
	{
		MessageBox( NULL, L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK ); ADLASSERT(0);
	}
	ADLASSERT( SUCCEEDED( hr ) );
...
//		TYPE_RTV = (1<<1),
	...
	__inline
	static void createRenderTarget( const Device* deviceData, int width, int height, DeviceRenderTargetDX11& renderTarget,
		u32 type = TYPE_RENDER_TARGET );
	__inline
	static void deleteRenderTarget( const Device* deviceData, DeviceRenderTargetDX11& renderTarget );
...
void DeviceRenderTargetDX11::createRenderTarget( const Device* deviceData, int width, int height, DeviceRenderTargetDX11& renderTarget,
	u32 type )
{
	DeviceDX11* dd = (DeviceDX11*)deviceData;
	...
	else if( type == TYPE_DEPTH_STENCIL )
	{
		...
		{
			D3D11_SHADER_RESOURCE_VIEW_DESC DescRV;
			DescRV.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS;//descDepth.Format;
			DescRV.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
			DescRV.Texture2D.MipLevels = 1;
			DescRV.Texture2D.MostDetailedMip = 0;
			dd->m_device->CreateShaderResourceView( renderTarget.m_texture, &DescRV, &renderTarget.m_srv );
...
void DeviceRenderTargetDX11::deleteRenderTarget( const Device* deviceData, DeviceRenderTargetDX11& renderTarget )
{
...
template<typename T, BufferBase::BufferType BUFFERTYPE>
struct AppendableBuffer
{
	AppendableBuffer( const DeviceDX11* deviceData );
	~AppendableBuffer();
...
template<typename T, BufferBase::BufferType BUFFERTYPE>
AppendableBuffer<T, BUFFERTYPE>::AppendableBuffer( const DeviceDX11* deviceData )
: m_deviceData( deviceData )
{
	reset();
...
//		DeviceUtilsDX11::createDeviceBuffer<T>( m_deviceData, BUFFER_CAPACITY, m_buffer, BUFFERTYPE );
		((DeviceDX11*)m_deviceData)->allocate( &m_buffer, BUFFER_CAPACITY, BUFFERTYPE );
}
...
//	DeviceUtilsDX11::deleteDeviceBuffer( m_deviceData, m_buffer );
	((DeviceDX11*)m_deviceData)->deallocate( &m_buffer );
}

template<typename T, BufferBase::BufferType BUFFERTYPE>
int AppendableBuffer<T, BUFFERTYPE>::append( int numElems, const void* hostPtr )
{
	ADLASSERT( m_nElems+numElems < BUFFER_CAPACITY );
	int nElems = m_nElems;

	m_buffer.write( (T*)hostPtr, numElems, m_nElems );
...
typedef AppendableBuffer<VertexColorStruct, BufferBase::BUFFER_VERTEX> AppendVertexBuffer;
typedef AppendableBuffer<u32, BufferBase::BUFFER_INDEX> AppendIndexBuffer;
...
struct RenderObject
{
	struct Resource
	{
		...
		Type m_type;
		BufferDX11<float4> m_buffer;

		void bind(const DeviceDX11* dd, int resIdx[5])
		{
			switch (m_type)
			{
			case VTX_SHADER:
				dd->m_context->VSSetShaderResources( resIdx[0]++, 1, m_buffer.getSRVPtr() );
				break;
			case GEOM_SHADER:
				dd->m_context->GSSetShaderResources( resIdx[1]++, 1, m_buffer.getSRVPtr() );
				break;
			case HULL_SHADER:
				dd->m_context->HSSetShaderResources( resIdx[2]++, 1, m_buffer.getSRVPtr() );
				break;
			case DOMAIN_SHADER:
				dd->m_context->DSSetShaderResources( resIdx[3]++, 1, m_buffer.getSRVPtr() );
				break;
			case PIXEL_SHADER:
				dd->m_context->PSSetShaderResources( resIdx[4]++, 1, m_buffer.getSRVPtr() );
				break;
			default:
				ADLASSERT(0);
				break;
	...
	virtual
	void render(const DeviceDX11* deviceData, ID3D11Buffer* globalConstBuffer)
	{
		...
		deviceData->m_context->PSSetConstantBuffers( 0, 1, &globalConstBuffer );

		deviceData->m_context->DrawIndexed( m_nIndices, m_idxOffset, m_vtxOffset );
	}

	void drawCall(const DeviceDX11* deviceData)
	{
		switch( m_drawType )
		{
		case DRAW_INDEXED:
...
__inline
void drawPointSprite(BufferDX11<float4>& vtxBuffer, BufferDX11<float4>& colorBuffer, int nVtx, float radius)
{
	RenderObject& obj = g_debugRenderObj.expandOne();
	new (&obj)RenderObject();
	obj.m_topology = D3D11_PRIMITIVE_TOPOLOGY_POINTLIST;
...
__inline
void drawAabb(const float4& ma, const float4& mi, const float4& color)
{
	XMFLOAT4 p[] = { XMFLOAT4(mi.x, mi.y, mi.z,0), XMFLOAT4(mi.x, mi.y, ma.z,0),
		XMFLOAT4(ma.x, mi.y, ma.z,0), XMFLOAT4(ma.x, mi.y, mi.z,0),
		
		XMFLOAT4(mi.x, ma.y, mi.z,0), XMFLOAT4(mi.x, ma.y, ma.z,0),
		XMFLOAT4(ma.x, ma.y, ma.z,0), XMFLOAT4(ma.x, ma.y, mi.z,0) };
...
RenderObject createQuad1(const float4& orig, const float4& extent, const float4& minTexCrd, const float4& maxTexCrd, float cx=1.f, float cy=1.f, float cz=1.f, float cw=1.f)
{
	...
	vtxWc[0].m_texCrd = XMFLOAT4(minTexCrd.x,maxTexCrd.y,0,0);
	vtxWc[1].m_texCrd = XMFLOAT4(maxTexCrd.x,maxTexCrd.y,0,0);
	vtxWc[2].m_texCrd = XMFLOAT4(minTexCrd.x,minTexCrd.y,0,0);
	vtxWc[3].m_texCrd = XMFLOAT4(maxTexCrd.x,minTexCrd.y,0,0);
...
__inline
void renderFullQuad(const Device* deviceData, DeviceShaderDX11* pixelShader, const float4& constValues,
	float s=1.f, float cx=1.f, float cy=1.f, float cz=1.f, float cw=1.f)
{
	DeviceDX11* dd = (DeviceDX11*)deviceData;

	RenderObject quad = createQuad(s,cx,cy,cz,cw);
...
__inline
void dispatchRenderList( const Device* device, Array<RenderObject>* objArray, void* viewTransposeMatrix, void* projectionTransposeMatrix )
{
	DeviceShaderDX11 prevVShader;
	DeviceShaderDX11 prevPShader;
	DeviceShaderDX11 prevGShader;
	DeviceShaderDX11 prevHShader;
	DeviceShaderDX11 prevDShader;
	void* prevVtxBuffer = 0;
	void* prevIdxBuffer = 0;
	u32 prevTopology = 0;

	DeviceDX11* deviceData = (DeviceDX11*)device;

	deviceData->m_context->VSSetShader( g_defaultVertexShader.m_vShader, 0, 0 );
	deviceData->m_context->PSSetShader( g_defaultPixelShader.m_pShader, 0, 0 );
	deviceData->m_context->IASetInputLayout( g_defaultVertexShader.m_vertexLayout );

	for(int i=0; i<objArray->getSize(); i++)
	{
		RenderObject* obj = &(*objArray)[i];

		obj->m_matrix.m_view = *(XMMATRIX*)viewTransposeMatrix;
		obj->m_matrix.m_projection = *(XMMATRIX*)projectionTransposeMatrix;

		Kernel kernel;
		Launcher launcher( deviceData, &kernel );
		launcher.setConst<ConstantBuffer>( g_constBuffer, obj->m_matrix );

		if( obj->m_vertexShader != prevVShader )
		{
			deviceData->m_context->VSSetShader( obj->m_vertexShader.m_vShader, 0, 0 );
			deviceData->m_context->IASetInputLayout( obj->m_vertexShader.m_vertexLayout );
			prevVShader = obj->m_vertexShader;
		}
		if( obj->m_pixelShader != prevPShader )
		{
			deviceData->m_context->PSSetShader( obj->m_pixelShader.m_pShader, 0, 0 );
			deviceData->m_context->PSSetConstantBuffers( 0, 1, g_constBuffer.getBufferPtr() );
			prevPShader = obj->m_pixelShader;
		}
		if( obj->m_geomShader != prevGShader )
		{
			deviceData->m_context->GSSetShader( obj->m_geomShader.m_gShader, 0, 0 );
			deviceData->m_context->GSSetConstantBuffers( 0, 1, g_constBuffer.getBufferPtr() );
			prevGShader = obj->m_geomShader;
		}
		if( obj->m_hullShader != prevHShader )
		{
			deviceData->m_context->HSSetShader( obj->m_hullShader.m_hShader, 0, 0 );
			prevHShader = obj->m_hullShader;
		}
		if( obj->m_domainShader != prevDShader )
		{
			deviceData->m_context->DSSetShader( obj->m_domainShader.m_dShader, 0, 0 );
			deviceData->m_context->DSSetConstantBuffers( 0, 1, g_constBuffer.getBufferPtr() );
			prevDShader = obj->m_domainShader;
		}

		int resIdx[5] = {0,0,0,0,0};

		for(int ir=0; ir<obj->m_nResources; ir++)
		{
			obj->m_resources[ir].bind( deviceData, resIdx );
		}

		if( prevVtxBuffer != obj->m_vtxBuffer )
		{
			UINT stride = (obj->m_vtxStride)? obj->m_vtxStride : sizeof( VertexColorStruct );
			UINT offset = 0;
			deviceData->m_context->IASetVertexBuffers( 0, 1, &obj->m_vtxBuffer, &stride, &offset );
			prevVtxBuffer = obj->m_vtxBuffer;
		}

		if( prevIdxBuffer != obj->m_idxBuffer )
		{
			deviceData->m_context->IASetIndexBuffer( obj->m_idxBuffer, DXGI_FORMAT_R32_UINT, 0 );
			prevIdxBuffer = obj->m_idxBuffer;
		}
		if( prevTopology != obj->m_topology )
		{
			deviceData->m_context->IASetPrimitiveTopology( obj->m_topology );
			prevTopology = obj->m_topology;
		}

		if( i==0 )
			deviceData->m_context->VSSetConstantBuffers( 0, 1, g_constBuffer.getBufferPtr() );

		obj->drawCall( deviceData );
	}

	{
		if( prevVShader.m_vShader ) deviceData->m_context->VSSetShader( NULL, 0, 0 );
		if( prevPShader.m_vShader ) deviceData->m_context->PSSetShader( NULL, 0, 0 );
		if( prevGShader.m_vShader ) deviceData->m_context->GSSetShader( NULL, 0, 0 );
		if( prevHShader.m_vShader ) deviceData->m_context->HSSetShader( NULL, 0, 0 );
		if( prevDShader.m_vShader ) deviceData->m_context->DSSetShader( NULL, 0, 0 );
	}
}
	...
	void* viewMatrix, void* projectionMatrix)
{
	...
	XMVECTOR At = XMVectorSet( at.x, at.y, at.z, at.w );
	XMVECTOR Up = XMVectorSet( up.x, up.y, up.z, up.w );

	if( TRANSPOSE )
		*(XMMATRIX*)viewMatrix = XMMatrixTranspose( XMMatrixLookAtLH( Eye, At, Up ) );
	else
		*(XMMATRIX*)viewMatrix = XMMatrixLookAtLH( Eye, At, Up );

	if( TRANSPOSE )
		*(XMMATRIX*)projectionMatrix = XMMatrixTranspose( XMMatrixPerspectiveFovLH( fov, aspectRatio, nearPlane, farPlane ) );
	else
