	...
	case TYPE_CL:
		{
			...
			cl_int e;
			m_ptr = clEnqueueMapBuffer( dcl->m_commandQueue, (cl_mem)buffer.m_ptr, false, CL_MAP_READ | CL_MAP_WRITE, 0, sizeof(T)*nElems, 0,0,0,&e );
...
void MapBuffer::unmap(const Buffer<T>& buffer, int nElems)
{
	...
	case TYPE_CL:
		{
			const DeviceCL* dcl = (const DeviceCL*)m_device;
			cl_int e = clEnqueueUnmapMemObject( dcl->m_commandQueue, (cl_mem)buffer.m_ptr, m_ptr, 0,0,0 );
			ADLASSERT( e == CL_SUCCESS );
		}
		break;
	default:
