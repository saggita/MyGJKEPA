...
static char* spPlatformVendor[3] = {	
	"Advanced Micro Devices, Inc.", 	
	"Intel(R) Corporation",
	"NVIDIA Corporation"	
};
...
cl_context btOclCommon::createContextFromType(cl_device_type deviceType, cl_int* pErrNum, intptr_t pGLContext, intptr_t pGLDC )
{
	...
	{        
		...
		{            
			...
			// Loop over the known manufactures that support OpenCL.  For GPU contexts,
			// only the first two manufacturers provide a GPU implementation.  All
			// three manufacturers support a CPU implementation.
			unsigned j, numKnownPlatforms = (deviceType == CL_DEVICE_TYPE_CPU) ? 2 : 3;
			for (j=0; j<numKnownPlatforms; j++)
			{
				if(!strcmp(pbuf, spPlatformVendor[j]))             
				{   
					bFound = true; 
					break;
				}
