...
typename ChNarrowphase<TYPE>::Data* ChNarrowphase<TYPE>::allocate( const Device* device )
{
	...
	data->m_supportCullingKernel = device->getKernel( PATH, KERNEL0, options, 0 );
	data->m_narrowphaseKernel = device->getKernel( PATH, KERNEL1, options, 0 );
	data->m_narrowphaseWithPlaneKernel = device->getKernel( PATH, "NarrowphaseWithPlaneKernel", options, 0 );// , false );
