...
__constant int SPHERE_SHAPE_PROXYTYPE = 8;
		...
		if( numObjects > 0 )
		{
			...
			for( int collision = collisionObjectIndices.firstObject; collision < collisionObjectIndices.endObject; ++collision )
			{
				...
				if( shapeDescription.collisionShapeType == CAPSULE_SHAPE_PROXYTYPE )
				{
					...
					float distanceFromLine = mylength3(position - closestPoint);
					...
					if( dist < minDistance )
					{
						...
						position = position + (float4)((minDistance - dist)*normalVector);
						velocity = (position - previousPosition) * velocityCoefficient * isolverdt;
				...
				else if( shapeDescription.collisionShapeType == SPHERE_SHAPE_PROXYTYPE )
				{
					// Colliding with a sphere
					float sphereRadius = shapeDescription.radius;
					float sphereMargin = shapeDescription.margin;
					
					float4 worldC1 = shapeDescription.shapeTransform[3];
					float dist = mylength3(worldC1 - position);
					float4 normalVector = (float4)(normalize(position - worldC1).xyz, 0.f);
						
					float4 colliderLinearVelocity = shapeDescription.linearVelocity;
					float4 colliderAngularVelocity = shapeDescription.angularVelocity;
					float4 velocityOfSurfacePoint = colliderLinearVelocity + cross(colliderAngularVelocity, position - worldC1);

					float minDistance = sphereRadius + sphereMargin;
					...
					if( dist < minDistance )
					{
						...
						float approachSpeed = dot(relativeVelocity, normalVector);
						float4 tangentVelocity = relativeVelocity - approachSpeed * normalVector;
						
						float4 frictionVector = tangentVelocity * (colliderFriction*clothFriction);
