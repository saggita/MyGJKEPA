	...
	__local float *vertexInverseMassSharedData)
{
	...
	{	
		...
		do
		{
			...
			float w0 = position0.w;
			float w1 = position1.w;
			...
			if ( (int)position0.w >= 0 )
			{
				inverseMass0 = 0.0;
			}

			if ( (int)position1.w >= 0 )
			{
				inverseMass1 = 0.0;
			}
		...
		for( int vertex = laneInWavefront; vertex < verticesUsedByWave; vertex+=WAVEFRONT_SIZE )
		{
			...
			g_vertexPositions[vertexAddress] = (float4)(vertexPositionSharedData[localWavefront*MAX_NUM_VERTICES_PER_WAVE + vertex].xyz, g_vertexPositions[vertexAddress].w);
		}		
