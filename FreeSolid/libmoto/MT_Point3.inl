...
GEN_INLINE MT_Point3 MT_Point3::min(const MT_Point3& p) const { 
    return MT_Point3(GEN_min(m_co[0], p[0]), 
					 GEN_min(m_co[1], p[1]),
					 GEN_min(m_co[2], p[2]));
}

GEN_INLINE MT_Point3 MT_Point3::max(const MT_Point3& p) const { 
    return MT_Point3(GEN_max(m_co[0], p[0]), 
					 GEN_max(m_co[1], p[1]),
					 GEN_max(m_co[2], p[2]));
}

GEN_INLINE void MT_Point3::set_min(const MT_Point3& p) { 
    GEN_set_min(m_co[0], p[0]);
	GEN_set_min(m_co[1], p[1]);
	GEN_set_min(m_co[2], p[2]);
}

GEN_INLINE void MT_Point3::set_max(const MT_Point3& p) { 
    GEN_set_max(m_co[0], p[0]);
	GEN_set_max(m_co[1], p[1]);
	GEN_set_max(m_co[2], p[2]);
}
...
GEN_INLINE bool operator<=(const MT_Point3& p1, const MT_Point3& p2) {
    return p1[0] <= p2[0] && p1[1] <= p2[1] && p1[2] <= p2[2];
}

GEN_INLINE MT_Point3 MT_min(const MT_Point3& p1, const MT_Point3& p2) { 
    return p1.min(p2);
}

GEN_INLINE MT_Point3 MT_max(const MT_Point3& p1, const MT_Point3& p2) { 
    return p1.max(p2);
}
